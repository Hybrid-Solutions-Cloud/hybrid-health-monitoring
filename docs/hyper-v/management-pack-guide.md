...
upgraded, and removed. Version `1.3.8.0` ("Private Cloud Powered by Hyper-V: A 360° View") is permanently sealed with public key token
...
**[Download version 1.3.8.0 now](../downloads/hyper-v-private-cloud.md).** Once imported,
...
installation to four files. The `1.3.8.0` deployment ZIP contains all 12 non-PureStorage solution
...
The current HCS product release is `1.3.8.0`. Independently, its S2D adapter references the lowest
...
When upgrading from 1.3.7.0, import all selected 1.3.8.0 sealed packs together and retain every
customer-owned override MP. Confirm the expected service-root membership and aspect-specific health
paths, then use the generated day-2 support catalog to trace any unhealthy leaf. Environments still upgrading
from 1.3.4.0 with the
temporary `Hcs.HyperVPrivateCloud.Topology.Hotfix` must import the new sealed packs first, confirm
permanent discovery owns the full topology, and only then remove that hotfix. Never remove customer
overrides during either upgrade path.
...
`1.3.8.0` for the non-PureStorage deployment; any other version is a failed/partial import:
...
`Hyper-V-Private-Cloud-Monitoring-Deployment-1.3.8.0.zip` together. Do not change or remove the
...
cluster/counter query. Version 1.3.8.0 performs node, quorum, network, group, and CSV collection
locally so LocalSystem on a passive node does not authenticate remotely to the owner node. A
NoMatchingEventsFound result is treated as an empty event set, while access and missing-log errors
remain visible; CSV counters are skipped on clusters with no CSVs. An empty staged BoundaryId does
not classify a real cluster node as standalone—the script derives `cluster:<name>` from `Get-Cluster`.
...
The product version and token above are the facts for release `1.3.8.0`; confirm them against the
...
contains the canonical override templates generated for product `1.3.8.0` and token
`54d0fb1159995c86`.
...
Release `1.3.8.0` provides the complete 360° service model for Hyper-V Private Clouds. Below is the exhaustive reference of all components, managed classes, discoveries, unit monitors, performance rules, operator diagnostic tasks, and override parameters.
